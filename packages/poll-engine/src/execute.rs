...
mod tests {
    ...
    fn creates_poll() {
        ...
        ctx.env.block.time = current_time;
    ...
    fn equal_max_outcomes_ends_in_draw() {
        ...
        cast_vote(&mut ctx, params2).unwrap();
        ctx.info.sender = Addr::unchecked("voter_3");
        cast_vote(&mut ctx, params3).unwrap();
        ctx.info.sender = Addr::unchecked("voter_4");
        cast_vote(&mut ctx, params4).unwrap();
        let poll = polls().load(ctx.deps.storage, poll.id).unwrap();
        ...
        end_poll(&mut ctx, end_params).unwrap();
        let poll = polls().load(ctx.deps.storage, poll.id).unwrap();
